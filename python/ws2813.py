...
class WS2813:
    """
    Driver for WS2813 RGB LEDs. May be used for controlling single LED or chain
    of LEDs.
    Example of use:
        chain = WS2813(spi_bus=1, led_count=4)
        data = [
            (255, 0, 0),    # red
            (0, 255, 0),    # green
            (0, 0, 255),    # blue
    ...
    def __init__(self, led_count=1, intensity=1):
        """
        Params:
        ...
        * intensity = light intensity (float up to 1)
        ...
        self.empty_buf = bytearray()

        # SPI init
        self.spi = machine.SPI(1, 3200000, sck=machine.Pin(14), mosi=machine.Pin(13), miso=machine.Pin(12))
        
        # turn LEDs off
        self.show(self.empty_buf)
    ...
    def update_buf(self, data, start=0):
        ...
        Order of colors in buffer is changed from RGB to GRB because WS2813 LED
